...
class Iframe extends \Magento\Payment\Block\Form {
     ...
     * @param \Magento\Ccavenuepay\Helper\Hss $hssHelper
     * @param \Magento\Framework\Filesystem\Directory\ReadFactory $readFactory
     * @param \Magento\Framework\Module\Dir\Reader $reader
     * @param array $data
     */
    public function __construct(
    \Magento\Framework\View\Element\Template\Context $context, \Magento\Sales\Model\OrderFactory $orderFactory, \Magento\Checkout\Model\Session $checkoutSession, \Magento\Ccavenuepay\Helper\Hss $hssHelper, \Magento\Framework\Filesystem\Directory\ReadFactory $readFactory, \Magento\Framework\Module\Dir\Reader $reader, array $data = []
    ) {
    ...
    protected function _construct() {
        ...
        if (in_array($paymentCode, $this->_hssHelper->getHssMethods())) {
            ...
            $directory = $this->readFactory->create($this->reader->getModuleDir('', 'Magento_Ccavenuepay'));
            $file = $this->resolver->getTemplateFileName($templateFile, ['module' => 'Magento_Ccavenuepay']);
    ...
    protected function _getBlock() {
        if (!$this->_block) {
            $this->_block = $this->getLayout()->createBlock(
                    'Magento\\Ccavenuepay\\Block\\' . str_replace(
                            ' ', '\\', ucwords(str_replace('_', ' ', $this->_paymentMethodCode))
                    ) . '\\Iframe'
            );
            if (!$this->_block instanceof \Magento\Ccavenuepay\Block\Iframe) {
