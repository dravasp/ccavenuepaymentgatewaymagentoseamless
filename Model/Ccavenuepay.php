	    ...
	    if (!empty($order)) {
			...
			if (!empty($billing)) {
				$request->setBillingName($billing->getFirstname())//." ".$billing->getLastname())
                    ...
                    ->setBillingCountry($objectManager->create('Magento\Directory\Model\Country')->loadByCode($billing->getCountry_id())->getName())
				...
				$get_data['billing_country']	=	$request->getBillingCountry();
            ...
            if (!empty($shipping)) {
			    $request->setDeliveryName($shipping->getFirstname())//." ".$shipping->getLastname())
                        ...
                        ->setDeliveryTel($shipping->getTelephone())
                        ->setDeliveryCity($shipping->getCity())
                        ->setDeliveryState($shipping->getRegion())
                        ->setDeliveryZip($shipping->getPostcode())
                        ->setDeliveryCountry($objectManager->create('Magento\Directory\Model\Country')->loadByCode($shipping->getCountry_id())->getName());
				...
				$get_data['delivery_country']	=	$request->getDeliveryCountry();
				$get_data['delivery_tel']		=	$request->getDeliveryTel();
            }
            ...
            $get_data['card_type']          =   $payment->getAdditionalInformation('payment_type');
            if($get_data['card_type'] == 'NBK'){
                $get_data['card_name']       =   $payment->getAdditionalInformation('net_bank');
            }else if ($get_data['card_type'] == 'DBCRD') {
                $get_data['card_name']       =   $payment->getAdditionalInformation('debit_type');
            }else{
                $get_data['card_name']       =   $this->dataHelper->getCcCardName($payment->getData('cc_type'));
            }

            $get_data['payment_option']  =   'OPT'.$get_data['card_type'];

            //set card number
            $get_data['card_number']     =   $this->dataHelper->getSessionData('ccLast4') ? $this->dataHelper->getSessionData('ccLast4') : $payment->decrypt($payment->getData('cc_number_enc'));

            //set card expire month
            if($payment->getData('cc_exp_month') != '' && $payment->getData('cc_exp_month') > 0){
                $get_data['expiry_month']    =   ($payment->getData('cc_exp_month') < 10 ? '0'.$payment->getData('cc_exp_month') : $payment->getData('cc_exp_month'));
            }

            //set card expire year
            if($payment->getData('cc_exp_year') != '' && $payment->getData('cc_exp_year') > 0){
                $get_data['expiry_year']     =   $payment->getData('cc_exp_year');
            }
            
            //set card cvv number
            $get_data['cvv_number']      =   $payment->decrypt($this->dataHelper->getSessionData('cardCvv'));
            
            $get_data['carddetails']     =   $payment->getData('cc_owner');
            
            //For vault
            if($this->getConfigData('ccavenue_vault') == 1){
                if($get_data['card_type'] == 'DBCRD' || $get_data['card_type'] == 'CRDC'){
                    if($this->dataHelper->getSessionData('payId')){
                        $get_data['customer_card_id'] = $this->dataHelper->getSessionData('payId');
                        $this->dataHelper->getSessionData('payId', true);
                    }else{
                        $get_data['saveCard'] =   $payment->getAdditionalInformation('save_card') != '' ? $payment->getAdditionalInformation('save_card') : 'N';
                    }
                    $get_data['customer_identifier'] = $get_data['merchant_id'].$order->getCustomerId();
                }
            }

            /*** Delete sessions ***/
            $this->dataHelper->getSessionData('cardCvv', true);
            $this->dataHelper->getSessionData('ccLast4', true);
		}
