...
use Magento\Ccavenuepay\Model\Request as CcavenuepayRequest;

/**
 * Authorize.net request model for Ccavenuepay model
 */
class Request extends CcavenuepayRequest {
    ...
    $merchantApiLoginId, $merchantTransactionKey, $amount, $currencyCode, $fpSequence, $fpTimestamp
    ) {
        return hash_hmac(
                "md5", $merchantApiLoginId . "^" . $fpSequence . "^" . $fpTimestamp . "^" . $amount . "^" . $currencyCode, $merchantTransactionKey
        );
    }

    /**
     * Set Ccavenuepay data to request.
     *
     * @param \Magento\Ccavenuepay\Model\Ccavenuepay $paymentMethod
     * @return $this
     */
    public function setConstantData(\Magento\Ccavenuepay\Model\Ccavenuepay $paymentMethod) {
        ...
        $this->setXLogin($paymentMethod->getConfigData('login'))
                ->setXMethod(\Magento\Ccavenuepay\Model\Ccavenuepay::REQUEST_METHOD_CC)
    ...
    \Magento\Sales\Model\Order $order, \Magento\Ccavenuepay\Model\Ccavenuepay $paymentMethod
    ) {
    ...
    public function signRequestData() {
        ...
        $hash = $this->generateRequestSign(
                $this->getXLogin(), $this->_getTransactionKey(), $this->getXAmount(), $this->getXCurrencyCode(), $this->getXFpSequence(), $fpTimestamp
        );
