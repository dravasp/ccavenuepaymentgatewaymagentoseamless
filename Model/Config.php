...
/**
 * Config model that is aware of all \Magento\Ccavenuepay payment methods
 * Works with Ccavenuepay-specific system configuration
...
class Config extends AbstractConfig {
    ...
    /**
     * Ccavenuepay Standard - alias METHOD_WPP_EXPRESS
     */
    const METHOD_WPS_EXPRESS = 'wps_express';

    /**
     * Ccavenuepay Standard Bml - alias METHOD_WPP_BML
     */
    const METHOD_WPS_BML = 'wps_express_bml';

    /**
     * Ccavenuepay Bill Me Later - Express Checkout
     */
    const METHOD_WPP_BML = 'ccavenuepay_express_bml';

    /**
     * Ccavenuepay Website Payments Pro - Direct Payments
     */
    const METHOD_WPP_DIRECT = 'Ccavenuepay_direct';

    /**
     * Ccavenuepay Website Payments Pro - Direct Payments - alias METHOD_PAYFLOWPRO
     */
    const METHOD_PAYMENT_PRO = 'Ccavenuepay_payment_pro';
    ...
    /**
     * Ccavenuepay Bill Me Later - Express Checkout (Payflow Edition)
     */
    ...
    const METHOD_BILLING_AGREEMENT = 'ccavenuepay_billing_agreement';
     ...
     */
    const XML_PATH_PAYPAL_EXPRESS_SKIP_ORDER_REVIEW_STEP_FLAG = 'payment/ccavenuepay_express/skip_order_review_step';
    ...
    protected $_buildNotationPPMap = [
        'ccavenuepay_express' => 'EC',
        'ccavenuepay_direct' => 'DP',
        'payflow_express' => 'EC',
    ];
    ...
    protected $_ecStyleConfigMap = [
        ...
        'ccavenuepay_hdrimg' => 'hdrimg',
        'ccavenuepay_hdrbordercolor' => 'hdrbordercolor',
        'ccavenuepay_hdrbackcolor' => 'hdrbackcolor',
        'ccavenuepay_payflowcolor' => 'payflowcolor',
    ];
     ...
     *
     * @var string[]
     * @link https://cms.ccavenuepay.com/us/cgi-bin/?cmd=_render-content&content_ID=developer/e_howto_api_ECButtonIntegration#id089QD0O0TX4__id08AH904I0YK
     */
    ...
    /**
     * @var \Magento\Ccavenuepay\Model\CertFactory
     */
    ...
    \Magento\Framework\App\Config\ScopeConfigInterface $scopeConfig, \Magento\Directory\Helper\Data $directoryHelper, \Magento\Store\Model\StoreManagerInterface $storeManager, \Magento\Payment\Model\Source\CctypeFactory $cctypeFactory, $params = []
    ) {
    ...
    public function getMerchantCountry() {
        $countryCode = $this->_scopeConfig->getValue(
                $this->_mapGeneralFieldset('merchant_country'), \Magento\Store\Model\ScopeInterface::SCOPE_STORE, $this->_storeId
        );
        if (!$countryCode) {
    ...
    public function isOrderReviewStepDisabled() {
        return $this->_scopeConfig->getValue(
                        self::XML_PATH_PAYPAL_EXPRESS_SKIP_ORDER_REVIEW_STEP_FLAG, \Magento\Store\Model\ScopeInterface::SCOPE_STORE, $this->_storeId
        );
    }
    ...
    public function getPaymentMarkImageUrl($localeCode, $orderTotal = null, $pal = null, $staticSize = null) {
        ...
        return sprintf(
                'https://www.ccavenuepayobjects.com/webstatic/en_US/i/buttons/pp-acceptance-%s.png', $staticSize
        );
    }
    ...
    public function getSolutionImageUrl($localeCode, $isVertical = false, $isEcheck = false) {
        return sprintf(
                'https://www.ccavenuepay.com/%s/i/bnr/%s_solution_PP%s.gif', $this->_getSupportedLocaleCode($localeCode), $isVertical ? 'vertical' : 'horizontal', $isEcheck ? 'eCheck' : ''
        );
    }
    ...
    public function getAdditionalOptionsLogoUrl($localeCode, $type = false) {
        $configType = $this->_scopeConfig->getValue(
                $this->_mapGenericStyleFieldset('logo'), \Magento\Store\Model\ScopeInterface::SCOPE_STORE, $this->_storeId
        );
        if (!$configType) {
            return false;
        }
        ...
        return sprintf('https://www.ccavenuepayobjects.com/%s/i/bnr/bnr_%s.gif', $locale, $type);
    }
        ...
        if ($params['locale'] == 'en_US') {
            switch ($type) {
                case self::EC_BUTTON_TYPE_MARK:
                    return 'https://www.ccavenuepayobjects.com/webstatic/en_US/i/buttons/ppcredit-logo-medium.png';
                case self::EC_BUTTON_TYPE_SHORTCUT:
                default:
                    return 'https://www.ccavenuepayobjects.com/webstatic/en_US/i/buttons/checkout-logo-medium.png';
            }
        }
        return sprintf(
                'https://fpdbs%s.ccavenuepay.com/dynamicimageweb?%s', $this->getValue('sandboxFlag') ? '.sandbox' : '', http_build_query($params)
        );
    }
    ...
    protected function _mapWppFieldset($fieldName) {
        switch ($fieldName) {
            ...
            case 'button_flavor':
            case 'button_type':
                return "ccavenuepay/wpp/{$fieldName}";
            default:
                return null;
        }
    }
    ...
    protected function _mapGenericStyleFieldset($fieldName) {
        switch ($fieldName) {
            case 'logo':
            case 'page_style':
            case 'ccavenuepay_hdrimg':
            case 'ccavenuepay_hdrbackcolor':
            case 'ccavenuepay_hdrbordercolor':
            case 'ccavenuepay_payflowcolor':
                return "ccavenuepay/style/{$fieldName}";
            default:
                return null;
        }
    }
    ...
    /**
     * Map Ccavenuepay General Settings
     *
     * @param string $fieldName
     * @return string|null
     */
    protected function _mapGeneralFieldset($fieldName) {
        switch ($fieldName) {
            ...
            case 'merchant_country':
                return "ccavenuepay/general/{$fieldName}";
            default:
                return null;
        }
    }
    ...
    public function getBmlPublisherId() {
        return $this->_scopeConfig->getValue(
                        'payment/' . self::METHOD_WPP_BML . '/publisher_id', \Magento\Store\Model\ScopeInterface::SCOPE_STORE, $this->_storeId
        );
    }
    ...
    public function getBmlDisplay($section) {
        $display = $this->_scopeConfig->getValue(
                'payment/' . self::METHOD_WPP_BML . '/' . $section . '_display', \Magento\Store\Model\ScopeInterface::SCOPE_STORE, $this->_storeId
        );
        $bmlActive = $this->_scopeConfig->getValue(
                'payment/' . self::METHOD_WPP_BML . '/active', \Magento\Store\Model\ScopeInterface::SCOPE_STORE, $this->_storeId
        );
        $bmlUkActive = $this->_scopeConfig->getValue(
                'payment/' . self::METHOD_WPP_PE_BML . '/active', \Magento\Store\Model\ScopeInterface::SCOPE_STORE, $this->_storeId
        );
    ...
    public function getBmlPosition($section) {
        return $this->_scopeConfig->getValue(
                        'payment/' . self::METHOD_WPP_BML . '/' . $section . '_position', \Magento\Store\Model\ScopeInterface::SCOPE_STORE, $this->_storeId
        );
    }
    ...
    public function getBmlSize($section) {
        return $this->_scopeConfig->getValue(
                        'payment/' . self::METHOD_WPP_BML . '/' . $section . '_size', \Magento\Store\Model\ScopeInterface::SCOPE_STORE, $this->_storeId
        );
    }
