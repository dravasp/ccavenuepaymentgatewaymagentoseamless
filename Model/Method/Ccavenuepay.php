<?php
namespace Magento\Ccavenuepay\Model\Method;

/**
 * @method \Magento\Quote\Api\Data\PaymentMethodExtensionInterface getExtensionAttributes()
 * @SuppressWarnings(PHPMD.CouplingBetweenObjects)
 */
class Ccavenuepay extends \Magento\Payment\Model\Method\AbstractMethod {
        ...
        if (in_array($info->getCcType(), $availableTypes)) {
            if ($this->validateCcNum(
                ...
                $ccNumAndTypeMatches = isset(
                                ...
                                $ccTypeRegExpList[$info->getCcType()], $ccNumber
                );
                $ccType = $ccNumAndTypeMatches ? $info->getCcType() : 'OT';

                if (!$ccNumAndTypeMatches && !$this->otherCcType($info->getCcType())) {
                    $errorMsg = __('The credit card number does not match the credit card type.');
                }
            } else {
