...
define([
    ...
    return Class.extend({
        defaults: {
            ...
            /**
             * An attribute of the element responsible for the activation of the Ccavenuepay Bml (data attribute)
             */
            enableBmlCcavenuepay: '[data-enable="bml-api"]',
        ...
        initEvents: function () {
            _.each(this.config.events, function (elementEvents, selector) {
                ...
                var ccavenuepaysolution = this,
                        selectorButton = ccavenuepaysolution.$self.find(selector),
                        $self = ccavenuepaysolution.$self,
                        events = elementEvents;

                selectorButton.on(ccavenuepaysolution.systemEvent, function () {
                    _.each(events, function (elementEvent, name) {
                        ...
                        var predicate = elementEvent.predicate,
                                ...
                                predicateCallback = function (functionPredicate) {
                                    result = functionPredicate(ccavenuepaysolution, predicate.message, predicate.argument);
                        ...
                        if (ccavenuepaysolution.getValue($(this)) === elementEvent.value) {
                            if (predicate.name) {
                                require([
                                    'Magento_Ccavenuepay/js/predicate/' + predicate.name
                                ], predicateCallback);
                            } else {
                                $self.trigger(name);
                            }
                    ...
                    _.each(instances, function (instance) {
                        var handler = new Rule({
                            ...
                            data: {
                                ...
                                enableBmlCcavenuepay: this.enableBmlCcavenuepay,
                                ccavenuepaysolutionsElements: this.solutionsElements,
        ...
        /**
         * Assign ccavenuepaysolutions elements
         *
         * @param {Object} elements
         * @returns {exports.setSolutionsElements}
         */
        setSolutionsElements: function (elements) {
            this.ccavenuepaysolutionsElements = elements;
